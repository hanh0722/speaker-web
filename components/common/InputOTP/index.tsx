...
const InputOTP: FC<InputOTPProps> = (props) => {
  ...
  const [otp, setOTP] = useState<Array<null | number>>(generateArray(inputs).map(_ => null));
  const [ref] = useState(generateArray(inputs).map(_ => createRef<HTMLInputElement>()));
  ...
  const positionNotHaveValue = (currentIndex: number) => {
    return otp.findIndex((value, index) => !value && index !== currentIndex);
  };
  ...
  const onKeyDownHandler = (event: React.KeyboardEvent<HTMLInputElement>, index: number) => {
    const isExistedOTP = otp.some((value, p) => p === index && !!value);
    if (isExistedOTP && (event.code === BACKSPACE || (event.ctrlKey && event.key === Z) || (event.metaKey && event.key === Z))){
