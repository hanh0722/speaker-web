      ...
      <BoxLayout title="Billing Address">
        <div className={styles.info}>
          ...
          <p className={styles.address}>
            Address: <span className="color-gray">{address?.info?.address} - {address?.info?.country}</span>
          </p>
          <p>Phone: <span className="color-gray">{address?.info?.phone_number}</span></p>
        </div>
      </BoxLayout>
      <BoxOrder/>
      <Button type="submit" variant="outlined" prefix="normal" fullWidth size="large" className={styles.btn} color="inherit">Complete Order</Button>
    </div>
  );
};
